...
iso.3.6.1.2.1.10.166.8.1.3.1.8.270 = STRING: "`)), func(oid Oid, value Variable) error {
		count++
		if 1 == count {
			if "1.3.6.1.2.1.10.166.8.1.3.1.8.267" != oid.ToString() ||
				"[octets]04410d01" != value.String() {
				t.Error(oid.String(), value)
			}
		} else if 2 == count {
			if "1.3.6.1.2.1.10.166.8.1.3.1.8.268" != oid.ToString() ||
				"[octets]0d0a28642f" != value.String() {
				t.Error(oid.String(), value)
			}
		} else if 3 == count {
			if "1.3.6.1.2.1.10.166.8.1.3.1.8.269" != oid.ToString() ||
				"[octets]0a14640f" != value.String() {
				t.Error(oid.String(), value)
			}
		} else if 4 == count {
			if "1.3.6.1.2.1.10.166.8.1.3.1.8.270" != oid.ToString() ||
...
// func ReadSnmpValue(addr, oid string, action PduType) (Variable, error) {
// 	cl, e := NewSnmpClient(addr)
// 	if nil != e {
// 		return nil, errors.New("create snmp client failed - " + e.Error())
// 	}
// 	defer cl.Close()
// 	req, e := cl.CreatePDU(action, V2c)
// 	if nil != e {
